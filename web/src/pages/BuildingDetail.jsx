...
export default function BuildingDetail() {
  ...
  const filteredIssues = issues.filter(issue => {
    const isActive = issue.issueStatus !== 'RESOLVED';
  ...
  const activeIssuesCount = issues.filter(i => i.issueStatus !== 'RESOLVED').length;
  const resolvedIssuesCount = issues.filter(i => i.issueStatus === 'RESOLVED').length;
  const highCount = issues.filter(i => i.issuePriority === 'HIGH' && i.issueStatus !== 'RESOLVED').length;
  const mediumCount = issues.filter(i => i.issuePriority === 'MEDIUM' && i.issueStatus !== 'RESOLVED').length;
  const lowCount = issues.filter(i => i.issuePriority === 'LOW' && i.issueStatus !== 'RESOLVED').length;
